...
export const mainBindings = function (ipcMain, fs) {
  ipcMain.on(readFileRequest, (_IpcMainEvent, args) => {
    const callback = function (error, data) {
      this.send(readFileResponse, {
        key: args.key,
        error,
        data:
          typeof data !== 'undefined' && data !== null ? data.toString() : ''
      });
    }.bind(_IpcMainEvent.sender);
    fs.readFile(args.filename, 'utf8', callback);
  });

  ipcMain.on(writeFileRequest, (_IpcMainEvent, args) => {
    const callback = function (error) {
      this.send(writeFileResponse, {
        keys: args.keys,
        error
      });
    }.bind(_IpcMainEvent.sender);
...
class Backend {
  ...
  init(services, backendOptions, i18nextOptions) {
    ...
    if (
      typeof backendOptions !== 'undefined' &&
      typeof backendOptions.contextBridgeApiKey !== 'undefined'
    ) {
    ...
    if (
      ...
      typeof window[`${contextBridgeApiKey}`].i18nextElectronBackend ===
        'undefined'
    ) {
      throw new Error(
        `'window.${contextBridgeApiKey}.i18nextElectronBackend' is not defined! Be sure you are setting up your BrowserWindow's preload script properly!`
      );
    }
    ...
    this.backendOptions = {
      ...
      i18nextElectronBackend:
        typeof window !== 'undefined'
          ? window[`${contextBridgeApiKey}`].i18nextElectronBackend
          : undefined
    };
    this.i18nextOptions = i18nextOptions;

    // log-related
    const logPrepend = '[i18nextElectronBackend:';
    ...
    if (typeof window !== 'undefined') {
      if (typeof this.backendOptions.i18nextElectronBackend === 'undefined') {
        console.error(
          `${this.rendererLog} i18nextElectronBackend is undefined, please ensure you are exposing i18nextElectronBackend via the contextBridge in your preload file.`
        );
        return;
      }
      ...
      const anonymous = function (error, data) {
        ...
        for (let j = 0; j < toWork[i].values.length; j++) {
          ...
          if (!keySeparator) {
            ...
            data = mergeNested(
              data,
              toWork[i].values[j].key,
              this.i18nextOptions.keySeparator,
              toWork[i].values[j].fallbackValue
            );
          }
        ...
        if (debug) {
          console.log(
            `${this.rendererLog} requesting the missing key '${key}' be written to file '${filename}'.`
          );
        }
